...
public class EnumDecoder extends ByteToMessageDecoder {
  ...
  public EnumDecoder(int maxObjectLength) {
    this(maxObjectLength, false);
  }

  public EnumDecoder(boolean streamArrayElements) {
    this(1024 * 1024, streamArrayElements);
  }

  public EnumDecoder(int maxObjectLength, boolean streamArrayElements) {
  ...
  protected void
    ...
    for (/* use current idx */; idx < wrtIdx; idx++) {
      ...
      if (state == ST_DECODING_NORMAL) {
        ...
        if (openBraces == 0) {
          ByteBuf json = extractObject(ctx, in, in.readerIndex(),
        ...
        if (!insideString
          ...
          for (int i = in.readerIndex(); Character
              .isWhitespace(in.getByte(i)); i++) {
            in.skipBytes(1);
          }
        ...
        // JSON object/array detected. Accumulate bytes until all braces/brackets are
        // closed.
      } else if (c == '{' || c == '[') {
        initDecoding(c);
