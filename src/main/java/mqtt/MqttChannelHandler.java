...
final class MqttChannelHandler
  ...
  MqttChannelHandler(MqttClientImpl client,
  ...
  public void channelActive(ChannelHandlerContext ctx) throws Exception {
    ...
    ctx.channel().writeAndFlush(
        new MqttConnectMessage(fixedHeader, variableHeader, payload));
  }
  ...
  private void invokeHandlersForIncomingPublish(MqttPublishMessage message) {
    ...
    for (Map.Entry<String, List<MqttSubscribtion>> entry : this.client
  ...
  private void handleSubAck(MqttSubAckMessage message) {
    MqttPendingSubscribtion pendingSubscribtion = this.client
    ...
    for (MqttPendingSubscribtion.MqttPendingHandler handler : pendingSubscribtion
      ...
      if (this.client.getSubscriptions()
          .containsKey(pendingSubscribtion.getTopic())) {
        this.client.getSubscriptions().get(pendingSubscribtion.getTopic())
            .add(subscribtion);

      } else {
        List<MqttSubscribtion> li = new LinkedList<>();
        li.add(subscribtion);
        this.client.getSubscriptions().put(pendingSubscribtion.getTopic(), li);
      }

      if (this.client.getHandlerToSubscribtion()
          .containsKey(handler.getHandler())) {
        this.client.getHandlerToSubscribtion().get(handler.getHandler())
            .add(subscribtion);
      } else {
        List<MqttSubscribtion> li = new LinkedList<>();
        li.add(subscribtion);
        this.client.getHandlerToSubscribtion().put(handler.getHandler(), li);
      }

    }
    this.client.getPendingSubscribeTopics()
  ...
  private void handlePublish(Channel channel, MqttPublishMessage message) {
    ...
    case AT_LEAST_ONCE:
      ...
      if (message.variableHeader().messageId() != -1) {
        ...
        channel
            .writeAndFlush(new MqttPubAckMessage(fixedHeader, variableHeader));
      }
      break;
  ...
  private void handleUnsuback(MqttUnsubAckMessage message) {
    MqttPendingUnsubscribtion unsubscribtion =
        this.client.getPendingServerUnsubscribes()
  ...
  private void handlePuback(MqttPubAckMessage message) {
    ...
    this.client.getPendingPublishes()
        .remove(message.variableHeader().messageId());
    pendingPublish.getPayload().release();
  }

  private void handlePubrec(Channel channel, MqttMessage message) {
    MqttPendingPublish pendingPublish = this.client.getPendingPublishes().get(
  ...
  private void handlePubrel(Channel channel, MqttMessage message) {
    if (this.client.getQos2PendingIncomingPublishes()
      ...
      this.client.getQos2PendingIncomingPublishes().remove(incomingQos2Publish
          .getIncomingPublish().variableHeader().messageId());
    }
  ...
  private void handlePubcomp(MqttMessage message) {
    ...
    MqttPendingPublish pendingPublish =
        this.client.getPendingPublishes().get(variableHeader.messageId());
    pendingPublish.getFuture().setSuccess(null);
